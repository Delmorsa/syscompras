<script>
    ...
    function autorizar(){
        ...
        "ordering": false,
		"ajax": {
			"url": "<?php echo base_url("index.php/editOrders/").$this->uri->segment(2)."/".$this->uri->segment(3)."/".$this->uri->segment(4)?>",
			"type": "POST"
		},
        columns: [
        ...
        if(valido){
            ...
            if(!retiene){
				...
				t.rows().eq(0).each(function (index) {
                    ...
                    if(bandera){
                        let form_data = {
                            enc: [$("#idSolicitud").val(),$("#listProv option:selected").val(),$("#Proveedor").val(),$("#fechaOP").val(),$("#cantidadTxt").val(),$("#cantidad").val(),$("#concepto").val(),porcRetiene,comentRetiene,$("#Chequetxt").val(),$("#idorden").val()],
                       ...
                       $.ajax({
                            url: "<?php echo base_url("index.php/updateOrdenPago")?>",
                            type: "POST",
                            data: form_data,
    ...
    $("#sumaTotalFoot").on("keyup",function (){
        let valOriginal = $("#cantidadTxt1").val();
        let valor = Number($(this).val());
        let valsuma = Number(valOriginal) + valor;
        $("#sumaTotalFoot").val(valsuma - valOriginal);
        $("#cantidad").val(valsuma - valOriginal);
        $("#cantidad").trigger("keyup");
        //console.log(valsuma);
	});
</script>
