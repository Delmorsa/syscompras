<script>
    ...
    function autorizar(){
        ...
        "ordering": false,
		"ajax": {
			"url": "<?php echo base_url("index.php/getSolicitudesDetOrden/").$this->uri->segment(2)?>",
			"type": "POST"
		},
        columns: [
    ...
    $("#listProv").on("change", function () {
           $("#Proveedor").val($("#listProv option:selected").text());
           $("#Chequetxt").val($("#listProv option:selected").text());
     });
        ...
        if(valido){
            ...
            if(!retiene){
				...
				t.rows().eq(0).each(function (index) {
                    ...
                    if(bandera){
                        let form_data = {
                            enc: [$("#idSolicitud").val(),$("#listProv option:selected").val(),$("#Proveedor").val(),$("#fechaOP").val(),$("#cantidadTxt").val(),$("#cantidad").val(),$("#concepto").val(),porcRetiene,comentRetiene,$("#Chequetxt").val()],
                       ...
                       $.ajax({
                            url: "<?php echo base_url("index.php/saveOrdenPago")?>",
                            type: "POST",
                            data: form_data,
    ...
    function aplicarMoneda(idDetalleSolicitud){
        if($("#lisMoneda"+idDetalleSolicitud).val() == "C"){
            $("#monedaValor1").text("CORDOBA");
            $("#monedaValor2").text("CORDOBAS");
            $("#cantidad").trigger("keyup");
        }else{
            $("#monedaValor1").text("DOLAR");
            $("#monedaValor2").text("DOLARES");
            $("#cantidad").trigger("keyup");
        }
    }

	$("#cantidad").on("keyup",function (){
		let num = numeroALetras($(this).val(), {
			plural: $("#monedaValor2").text(),
			singular: $("#monedaValor1").text(),
			centPlural: "CENTAVOS",
    ...
    $("#selectMonedaAll").change(function () { 
        let t = $("#tblAutSolicitudes").DataTable();
        //console.log(String(this.value));
        t.rows().eq(0).each(function (index) {
                let row = t.row(index);
                let datos = row.data();
                //console.log($("#lisMoneda"+datos.IdDetallesSolicitud+" option:selected").text().trim());   
                switch ($("#selectMonedaAll option:selected").val()) {
                    case "C":
                        $("#lisMoneda"+datos.IdDetallesSolicitud+" option:selected").text("C$").val($("#selectMonedaAll option:selected").val());
                        $("#lisMoneda"+datos.IdDetallesSolicitud).attr("disabled",true);
                        $("#lisMoneda"+datos.IdDetallesSolicitud).trigger("change");
                    break;
                    case "S":
                        $("#lisMoneda"+datos.IdDetallesSolicitud+" option:selected").text("$").val($("#selectMonedaAll option:selected").val());
                        $("#lisMoneda"+datos.IdDetallesSolicitud).attr("disabled",true);
                        $("#lisMoneda"+datos.IdDetallesSolicitud).trigger("change");
                        break;
                    default:
                        $("#lisMoneda"+datos.IdDetallesSolicitud+" option:selected").text("C$").val("C");
                        $("#lisMoneda"+datos.IdDetallesSolicitud).trigger("change");
                        break;
                }
                if($("#selectMonedaAll option:selected").text() == "Manual"){
                    $("#lisMoneda"+datos.IdDetallesSolicitud).removeAttr("disabled"); 
                }
        });
     });
</script>
