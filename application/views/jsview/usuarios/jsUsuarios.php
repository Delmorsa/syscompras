...
$("#btnAddUser").click(function(){
    ...
    $("#btnSoli").removeClass("active");
    $("#btnAut").removeClass("active");
    
    $("#chksolicita").prop("checked",false);
    $("#chkautoriza").prop("checked", false);

    $("#listRoles").val(null).trigger("change");

    $("#listAreas option:selected").val(null).trigger("change");

    $("#divJefes").hide();        
    $("#listJefes option:selected").val(null).trigger("change");
    $("#listJefes option:selected").text(null).trigger("change");
    $("#select2-listJefes-container").text(null).trigger("change");


    $("#modalUser").modal("show");
});

$('input[type=radio][name=permiso]').change(function() {
    if (this.value == 1) {
        $("#divJefes").show();        
        $("#listJefes").select2({
			...
			ajax: {
				...
				processResults: function (data) {
					let res = [];
					for(let i  = 0 ; i < data.length; i++) {
						res.push({id:data[i].IdUsuario, text:data[i].Nombre});
					}
					return {
						results: res
					}
...
$("#btnGuardarUser").click(function () {
	...
	if (valido) {
        ...
        if(bandera){
           ...
           let form_data = {
            idRol: $("#listRoles option:selected").val(),
...
function editar(iduser,user,nombre,puesto,correo,genero,solicita,aut,idrol,idarea,area,IdJefe,Jefe){//,idrol,rol,idarea,area,genero,solicita,aut){
    ...
    if(solicita == 1){
        $("#chkautoriza").prop("checked", false);
        $("#chksolicita").prop("checked",true);
        $("#btnSoli").addClass("active");
        $("#btnAut").removeClass("active");
        $("#divJefes").show();        
        $("#listJefes").select2({
			placeholder: '--- Seleccione un jefe ---',
        ...
        $("#listJefes option:selected").val(IdJefe).trigger("change");
        $("#listJefes option:selected").text(Jefe).trigger("change");
        $("#select2-listJefes-container").text(Jefe).trigger("change");
    }

    if(aut == 1){
        $("#chksolicita").prop("checked",false);
        $("#chkautoriza").prop("checked", true);
        $("#btnSoli").removeClass("active");
        $("#btnAut").addClass("active");
        $("#divJefes").hide();    
    }
    $("#listRoles").val(idrol).trigger("change");
