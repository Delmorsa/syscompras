...
class Usuarios_controller extends CI_Controller {
    ...
    public function getUserJefe(){
        ...
        $this->Usuarios_model->getUserJefe($var);
    }
    ...
    public function getUsuariosAjax(){
        ...
        foreach ($resultado->result_array() as $key) {
            $array = array();
            if($key["Solicita"] == 1){
                $solicita = '<span class="badge badge-success fs-7 fw-bold">Si</span>';	
            }else{
                $solicita = '<span class="badge badge-danger fs-7 fw-bold">No</span>';	
            }

            if($key["Autoriza"] == 1){
                $aut = '<span class="badge badge-success fs-7 fw-bold">Si</span>';	
            }else{
                $aut = '<span class="badge badge-danger fs-7 fw-bold">No</span>';	
            }
            ...
            $array["Rol"] = $key["NombreRol"];
            $array["Area"] = $key["NombreArea"];
            $array["Correo"] = $key["Correo"];
            $array["Solicita"] = $solicita;
            $array["Autoriza"] = $aut;
            $array["Estado"] = $estado;
            ...
            if($key["Estado"] == "A"){
                $array["Opciones"] = "
                    ...
                    ,"'.$key["Puesto"].'","'.$key["Correo"].'","'.$key["Genero"].'","'.$key["Solicita"].'","'.$key["Autoriza"].'"
                    ,"'.$key["IdRol"].'","'.$key["IdArea"].'","'.$key["NombreArea"].'","'.$key["IdJefe"].'","'.$key["Jefe"].'"'.")'
    ...
    public function guardarUser(){
        ...
        $solicita = $this->input->get_post("solicita");
        $autoriza = $this->input->get_post("autoriza");
        $jefe = $this->input->get_post("jefe");
        $this->Usuarios_model->guardarUser($idRol,$idArea,$user,$pass,$nombre,$puesto,$correo,$genero,$solicita,$autoriza,$jefe);
    }
    ...
    public function actualizarUser(){
        ...
        $autoriza = $this->input->get_post("autoriza");
        $jefe = $this->input->get_post("jefe");
        $this->Usuarios_model->actualizarUser($idUser,$idRol,$idArea,$user,$nombre,$puesto,$correo,$genero,$solicita,$autoriza,$jefe);
    }
...
/************************************************************************************************* */
    public function estadoSolcitudes(){
        $this->Usuarios_model->estadoSolcitudes();
    } 

    public function contadorSolic(){
        $this->Usuarios_model->contadorSolic();
    } 
